...
<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
                ...
                <x-filament::grid :default="$getGridColumns('default')" :sm="$getGridColumns('sm')" :md="$getGridColumns('md')" :lg="$getGridColumns('lg')"
                    ...
                    @foreach ($containers as $uuid => $item)
                        @php
                            ...
                            $itemDepth = $getItemDepth(['item' => $uuid]);
                            ...
                            $moveDownAction = $moveDownAction(['item' => $uuid])->disabled(
                                !$canMoveDown($itemDepth, @$previousItemDepth),
                            );
                            $moveDownActionIsVisible = $isReorderableWithButtons && $moveDownAction->isVisible();
                            $moveUpAction = $moveUpAction(['item' => $uuid])->disabled($itemDepth <= 0);
                            $moveUpActionIsVisible = $isReorderableWithButtons && $moveUpAction->isVisible();
                            $reorderActionIsVisible = $isReorderableWithDragAndDrop && $reorderAction->isVisible();

                            $previousItemDepth = $itemDepth;
                        @endphp
                        ...
                        <li wire:ignore.self
                            ...
                            x-sortable-item="{{ $uuid }}" class="fi-fo-repeater-item"
                            ...
                            @if (
                                ...
                                <div @if ($isCollapsible) x-on:click.stop="isCollapsed = !isCollapsed" @endif
                                    style="margin-left: {{ $itemDepth * 32 }}px;" @class([
    ...
    <script>
        document.addEventListener('open-edit-modal', event => {
            ...
            $wire.mountFormComponentAction('edit', {
